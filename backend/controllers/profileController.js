...
const getUserSkillBadges = async (userId) => {
  const completedSessions = await Session.find({
    $or: [{ learner_id: userId }, { participants: userId }],
    status: 'completed',
  }).select('skill_id duration skillClaimed skillClaimedBy session_level date session_topic').sort({ date: 1 }).lean();
  ...
  completedSessions.forEach(s => {
    ...
    if (s.skillClaimedBy && s.skillClaimedBy.some(id => id.toString() === userId.toString())) {
      hasClaimed = true;
    } else if (s.skillClaimed) {
      hasClaimed = true;
    }
    ...
    if (!skillMap[skill]) {
      skillMap[skill] = {
        skillName: skill,
        level: s.session_level || 'Beginner',
        sessionsCount: 0,
        claimed: false,
        sessions: [],
        hoursSpent: 0
    ...
    skillMap[skill].sessionsCount += 1;
    skillMap[skill].hoursSpent += s.duration || 1;
    skillMap[skill].sessions.push({
      id: s._id.toString(),
      topic: s.session_topic || 'General Topic',
      level: s.session_level || 'Beginner',
      date: s.date
    });

    if (hasClaimed) skillMap[skill].claimed = true;
    if (s.session_level) skillMap[skill].level = s.session_level;
  });

  return Object.values(skillMap).filter(b => b.claimed);
};
...
exports.masterSkill = async (req, res) => {
  try {
    ...
    if (session.status !== 'completed') {
      return res.status(400).json({ success: false, message: 'Session must be completed to end learning' });
    }
    ...
    if (!hasFeedback && !session.feedbackGiven) {
      return res.status(400).json({
        success: false,
        message: 'Please provide feedback for your teacher from the Activity page before ending learning for this skill'
      });
    }
    ...
    if (hasClaimedSkill) {
      return res.status(400).json({ success: false, message: 'You have already ended learning for this session' });
    }
    ...
    // ✅ NEW: Check if THIS user has already ended learning for this SKILL (not just this session)
    const skillId = session.skill_id;
    const alreadyEndedSkill = (profile.skillEndedBy || []).some(
      entry => entry.userId.toString() === userId.toString() && entry.skillId === skillId
    );
    if (alreadyEndedSkill) {
      return res.status(400).json({
        success: false,
        message: `You have already ended learning for "${skillId}". You can only end learning once per skill.`
      });
    }

    // Add this learner to skillClaimedBy array on the session (per-session flag)
    ...
    // ✅ Record per-skill end-learning for this user (prevents future duplicates for this skill)
    if (!profile.skillEndedBy) {
      profile.skillEndedBy = [];
    }
    profile.skillEndedBy.push({ userId, skillId });

    // Increment skillsMastered by exactly 1 (one per skill, not per session)
    profile.skillsMastered = (profile.skillsMastered || 0) + 1;
    await profile.save();

    res.status(200).json({
      success: true,
      message: 'Learning ended successfully',
...
exports.getProfileById = async (req, res) => {
  try {
    ...
    const skillBadges = await getUserSkillBadges(profile.userId?._id || req.user._id);
...
exports.getProfileByUserId = async (req, res) => {
  try {
    ...
    const skillBadges = await getUserSkillBadges(profile.userId?._id || userId);
...
exports.getProgress = async (req, res) => {
  try {
    ...
    const completedSessions = await Session.find({
      $or: [{ learner_id: userId }, { participants: userId }],
      status: 'completed'
    }).select('skill_id duration skillClaimed session_level date').sort({ date: 1 });
    ...
    completedSessions.forEach(s => {
      ...
      if (!skillMap[skill]) {
        skillMap[skill] = { hours: 0, sessions: 0, claimed: false, level: s.session_level || 'Beginner' };
      }
