...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find().populate('subscription.plan').lean();
    ...
    const formattedUsers = users.map(user => {
      ...
      const subscriptionPlan = user.subscription && user.subscription.plan;
      const subscriptionType = subscriptionPlan?.type || 'Free';
  ...
  try {
    if (!req.body) {
      return res.status(400).json({
        success: false,
        message: 'Request body is required (userId and status). Make sure you are sending JSON with Content-Type: application/json.',
      });
    }
...
exports.getDashboardStats = async (req, res) => {
  try {
    const [totalUsers, activeDisputes, successfulExchanges] = await Promise.all([
    ...
    const activeSubs = await User.find({ status: { $ne: 'Deleted' }, 'subscription.status': 'active' })
      .select('subscription.plan')
      .populate('subscription.plan')
      .lean();

    let premiumProfiles = 0;
    let professionalProfiles = 0;
    let monthlyRevenueRaw = 0;

    activeSubs.forEach(u => {
      const plan = u.subscription && u.subscription.plan;
      if (!plan) return;
      if (plan.type === 'Premium') premiumProfiles += 1;
      if (plan.type === 'Professional') professionalProfiles += 1;
      monthlyRevenueRaw += plan.price || 0;
    });

    const freeProfiles = Math.max(totalUsers - premiumProfiles - professionalProfiles, 0);
