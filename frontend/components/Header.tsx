...
const Header: React.FC<HeaderProps> = ({ isAuthenticated, navigation, currentPage }) => {
  ...
  useEffect(() => {
    // Close mobile search if we navigate away from the home page
        ...
        <div className="hidden lg:flex items-center space-x-6">
          ...
          {currentPage === Page.Home && (
            <div className="relative">
              <input type="text" placeholder="Search skills..." className="bg-white/20 text-white placeholder-gray-300 rounded-full py-1.5 pl-10 pr-4 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all" />
          ...
          {isAuthenticated ? (
            <div className="flex items-center space-x-4">
              <button className="text-gray-200 hover:text-white relative" aria-label="Notifications">
                <NotificationIcon className="w-6 h-6" />
                <span className="absolute top-0 right-0 block h-2 w-2 rounded-full bg-red-500 ring-2 ring-brand-teal"></span>
              </button>
              <button className="text-gray-200 hover:text-white" aria-label="Chat">
                <MessengerIcon className="w-6 h-6" />
              </button>
              <button onClick={() => navigateTo(Page.MyAccount)} className="text-gray-200 hover:text-white" aria-label="My Account">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={() => navigateTo(Page.Signup1)}
                className="bg-white text-brand-teal font-semibold px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors"
              >
                Sign Up
              </button>
            </div>
          ...
          {isMobileSearchOpen && currentPage === Page.Home ? (
            <div className="w-full flex items-center">
              ...
              <button
                ...
                className="text-white ml-2 shrink-0"
