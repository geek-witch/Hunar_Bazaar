    ...
    const fetchPlansAndSubscription = async () => {
      ...
      try {
        const [plansResponse, subResponse] = await Promise.all([
          ...
          subscriptionApi.getMySubscription().catch(() => ({ success: false })),
        ])
            ...
            {isLoadingPlans ? (
              ...
              plans.map((plan) => (
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      if (plan.type !== currentPlanType && plan.price > 0) {
                  ...
                  >
                    {plan.type === currentPlanType ? 'Your Current Plan' : `Upgrade to ${plan.name}`}
                  </button>
                </div>
              ))
            ) : (
