...
const SettingsPage: React.FC<{ navigation: Navigation }> = ({ navigation }) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setSubscription({
            ...
            pricePerMonth: data.price,
            renewalDate: data.currentPeriodEnd || null,
  ...
  const handleToggleAutoRenew = async () => {
    ...
    try {
      ...
      const response = await subscriptionApi.updateAutoRenew(nextAutoRenew)
      if (response.success && response.data) {
        setSubscription(prev => prev ? ({
          ...prev,
          autoRenew: nextAutoRenew,
          renewalDate: (response.data as any).currentPeriodEnd || prev.renewalDate,
          status: (response.data as any).status === "active" ? "active" : "canceled",
        }) : prev)
        setSavedMessage("Subscription settings updated")
        setTimeout(() => setSavedMessage(""), 3000)
      } else {
        navigation.showNotification(response.message || "Failed to update subscription settings")
      }
    } catch (error) {
      navigation.showNotification("Failed to update subscription settings")
    }
  }

  const handleCancelSubscription = async () => {
    if (!subscription) {
      navigation.showNotification("You don't have any active subscription to cancel.")
      return
    }

    if (subscription.status !== "active") {
      navigation.showNotification("Your subscription is already inactive or scheduled to end.")
      return
    }

    if (!subscription.autoRenew) {
      navigation.showNotification("Your subscription is already set to end at the current billing period.")
      return
    }

    try {
      // Use auto-renew toggle to cancel at period end, so remaining period is not wasted
      const response = await subscriptionApi.updateAutoRenew(false)
      if (response.success && response.data) {
        setSubscription(prev => prev ? ({
          ...prev,
          status: "canceled",
        ...
        setSavedMessage("Subscription will end at the current billing period")
        setTimeout(() => setSavedMessage(""), 3000)
      } else {
        navigation.showNotification(response.message || "Failed to cancel subscription")
      }
    } catch (error) {
      navigation.showNotification("Failed to cancel subscription")
    }
  }
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            {isLoadingSubscription ? (
              <div className="text-center text-sm text-slate-500">Loading subscription details...</div>
            ) : !subscription ? (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 bg-purple-50 rounded-2xl border border-purple-100">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-800 flex items-baseline gap-1">
                    Free
                    <span className="text-lg font-medium text-slate-500">— ₨0/mo</span>
                  </p>
                </div>
                <div className="text-left sm:text-right">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Status</p>
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-bold bg-slate-100 text-slate-600">
                  ...
                  <div>
                    ...
                    {subscription.renewalDate && (
                      <p className="text-xs text-slate-500 mt-1">
                        Next billing / end date: {new Date(subscription.renewalDate).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                  <div className="text-left sm:text-right">
                      ...
                      <button
                        onClick={async () => {
                          if (!subscription || subscription.status !== "active") {
                            navigation.showNotification("You need an active subscription to update payment details.")
                            return
                          }
                          const resp = await subscriptionApi.updatePaymentMethod()
                          if (resp.success && (resp as any).url) {
                            window.location.href = (resp as any).url
                          } else {
                            navigation.showNotification(resp.message || "Failed to open billing portal")
                          }
                        }}
                        className="flex-1 bg-white border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm"
                      >
