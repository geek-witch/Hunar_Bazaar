...
interface Request {
  ...
  skill: string
  message: string
  requestType: "incoming" | "outgoing" | "session_join"
...
const ManageRequestsPage: React.FC<{ navigation: Navigation }> = ({ navigation }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const sjReqs = await sessionApi.getSessionJoinRequests()
        ...
        // Load session join requests
        if (sjReqs.success && sjReqs.data) {
          setSessionJoinRequests(sjReqs.data)
        }
  ...
  const handleAcceptSessionJoinRequest = (requestId: string) => {
    ; (async () => {
      try {
        const resp = await sessionApi.respondSessionJoinRequest(requestId, 'accept')
        if (resp.success) {
          // Remove from pending requests
          setSessionJoinRequests(prev => prev.filter(r => r.id !== requestId))
          // Show success notification
          if (window.showNotification) {
            window.showNotification('User added to session and friends list!', 'success')
          }
        }
      } catch (e) {
        if (window.showNotification) {
          window.showNotification('Failed to accept request', 'error')
        }
      }
    })()
  }

  const handleRejectSessionJoinRequest = (requestId: string) => {
    ; (async () => {
      try {
        const resp = await sessionApi.respondSessionJoinRequest(requestId, 'reject')
        if (resp.success) {
          // Remove from pending requests
          setSessionJoinRequests(prev => prev.filter(r => r.id !== requestId))
          // Show success notification
  ...
  const SessionJoinRequestCard: React.FC<{ request: SessionJoinRequest }> = ({ request }) => (
    <div className="group bg-white p-6 sm:p-8 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 hover:shadow-2xl hover:shadow-cyan-100/50 transition-all duration-300 relative overflow-hidden">
      <div className="flex items-start justify-between mb-6 relative z-10">
        <div className="flex items-center gap-4 min-w-0">
          <div className="w-14 h-14 rounded-2xl p-1 bg-white shadow-sm border border-slate-100 relative">
            <img
              ...
              className="w-full h-full rounded-xl object-cover cursor-pointer"
              onClick={() => { sessionStorage.setItem('selectedProfileId', request.userId); navigation.navigateTo(Page.ViewProfile) }}
            />
          ...
          <div className="min-w-0">
            <h3 className="font-bold text-lg text-slate-800 truncate cursor-pointer hover:text-brand-teal transition-colors" onClick={() => { sessionStorage.setItem('selectedProfileId', request.userId); navigation.navigateTo(Page.ViewProfile) }}>{request.userName}</h3>
            <span className="inline-block px-3 py-1 bg-purple-50 text-purple-700 text-xs font-bold rounded-full mt-1 truncate max-w-full">
              {request.skill}
            </span>
          </div>
        </div>
        <span className="px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm ml-2 bg-purple-100 text-purple-700">
          Session Join
        </span>
      </div>
      ...
      <div className="flex items-center justify-between mb-6 text-xs font-medium text-slate-400 relative z-10">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            <span>{new Date(request.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
          </div>
          <div className="flex items-center gap-1.5">
      ...
      <div className="relative z-10 pt-2 border-t border-slate-100">
        <SlideAction
          onAccept={() => handleAcceptSessionJoinRequest(request.id)}
          onReject={() => handleRejectSessionJoinRequest(request.id)}
          acceptLabel="Accept"
          rejectLabel="Decline"
        />
      </div>
      ...
      {/* Decorative Gradient Blob */}
      <div className="absolute -top-10 -right-10 w-32 h-32 bg-gradient-to-br from-purple-100/50 to-pink-100/50 rounded-full blur-3xl -z-0"></div>
    </div>
  )
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-8">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-xl shadow-cyan-100/50">
        ...
        <div className="relative z-10 p-6 sm:p-8 flex flex-col items-center sm:items-start gap-3">
          ...
          <p className="text-cyan-50 text-base max-w-2xl">View your incoming connection requests and manage your peers list.</p>
        </div>
      </div>
      ...
      {/* Session Join Requests Section */}
      <div className="pt-4">
        <div className="flex items-center gap-4 mb-6">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            Session Join Requests
            <span className="text-white text-sm font-bold px-2.5 py-0.5 rounded-full shadow-sm">{sessionJoinRequests.length}</span>
          </h2>
        </div>
        ...
        {sessionJoinRequests.length === 0 ? (
          <div className="bg-white p-16 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">No Session Join Requests</h3>
            <p className="text-slate-500 text-lg">You don't have any session join requests at the moment.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {sessionJoinRequests.map((request) => (
              <SessionJoinRequestCard key={request.id} request={request} />
            ))}
          </div>
        )}
      </div>
