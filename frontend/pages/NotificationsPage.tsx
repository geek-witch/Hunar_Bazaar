...
const NotificationsPage: React.FC<{ navigation: Navigation }> = ({ navigation }) => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'plan_updated':
        return '🚀';
      case 'skill_based_session_available':
        return '✨';
      case 'session_join_request_new':
        return '🔔';
      case 'session_join_request_accepted':
        return '✅';
      case 'session_join_request_rejected':
        return '❌';
      default:
        return '🔔';
    }
            ...
            filteredNotifications.map(notif => (
              <div
                ...
                onClick={() => {
                  ...
                  } else if (notif.type === 'skill_based_session_available') {
                    // Store session and teacher IDs and navigate to teacher's profile
                    // The entityId contains the session ID
                    // The relatedUserId contains the teacher ID
                    if ((notif as any).entityId && (notif as any).relatedUserId) {
                      sessionStorage.setItem('viewSessionId', (notif as any).entityId);
                      sessionStorage.setItem('scrollToSession', 'true');
                      sessionStorage.setItem('selectedProfileId', (notif as any).relatedUserId);
                      navigation.navigateTo(Page.ViewProfile);
                    }
                  } else if (notif.type === 'session_join_request_new') {
                    // Teacher's dashboard to see join requests
                    navigation.setCurrentPage('MyAccount', { tab: 'Manage Requests' });
                  } else if (notif.type === 'session_join_request_accepted' || notif.type === 'session_join_request_rejected') {
                    if ((notif as any).relatedUserId) {
                      sessionStorage.setItem('selectedProfileId', (notif as any).relatedUserId);
                      navigation.navigateTo(Page.ViewProfile);
                    } else {
                      navigation.setCurrentPage('MyAccount', { tab: notif.type === 'session_join_request_accepted' ? 'Schedule Classes' : 'Watch Activity' });
                    }
                  }
