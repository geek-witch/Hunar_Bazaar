...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (hash.includes('subscription_success=true')) {
      const paramsString = hash.split('?')[1] || ''
      const searchParams = new URLSearchParams(paramsString)
      const sessionId = searchParams.get('session_id')

      if (sessionId) {
        // Remove query params from hash to avoid reprocessing
        const baseHash = hash.split('?')[0]
        window.location.hash = baseHash

        // Confirm subscription with backend
        import('./utils/api').then(({ subscriptionApi }) => {
          subscriptionApi.confirmCheckout(sessionId)
            .then((resp) => {
              if (resp.success) {
                setNotification({ message: 'Subscription updated successfully.' })
              } else if (resp.message) {
                setNotification({ message: resp.message })
              }
            })
            .catch(() => {
              setNotification({ message: 'Subscription may not be updated. Please refresh or contact support.' })
            })
        })
      }
    }
